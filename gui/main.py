...
from helpers import generate_summary, generate_tags, parse_ctakes_to_json
import json
            ...
            if enriched_text:
                ...
                if ctakes_response:
                    ...
                    try:
                        ...
                        with st.expander("Anatomical Sites", expanded=False):
                            if tags["anatomical_sites"]:
                                badges = "".join([f'<div class="pair-container"><span class="term-badge">{term}</span><span class="code-badge">{code}</span></div>' 
                                                for code, term in tags["anatomical_sites"]])
                        ...
                        with st.expander("Procedures", expanded=False):
                            if tags["procedures"]:
                                badges = "".join([f'<div class="pair-container"><span class="term-badge">{term}</span><span class="code-badge">{code}</span></div>' 
                                                for code, term in tags["procedures"]])
                                st.markdown(badges, unsafe_allow_html=True)
                            else:
                                st.write("No codes found.")
                        ...
                        with st.expander("Symptoms", expanded=False):
                            if tags["symptoms"]:
                                badges = "".join([f'<div class="pair-container"><span class="term-badge">{term}</span><span class="code-badge">{code}</span></div>' 
                                                for code, term in tags["symptoms"]])
                                st.markdown(badges, unsafe_allow_html=True)
                            else:
                                st.write("No codes found.")
                        ...
                        with st.expander("Diagnosis", expanded=False):
                            if tags["diagnosis"]:
                                badges = "".join([f'<div class="pair-container"><span class="term-badge">{term}</span><span class="code-badge">{code}</span></div>' 
                                                for code, term in tags["diagnosis"]])
                                st.markdown(badges, unsafe_allow_html=True)
                            else:
                                st.write("No codes found.")
                        ...
                        with st.expander("Medications", expanded=False):
                            if tags["medications"]:
                                badges = "".join([f'<div class="pair-container"><span class="term-badge">{term}</span><span class="code-badge">{code}</span></div>' 
                                                for code, term in tags["medications"]])
                                st.markdown(badges, unsafe_allow_html=True)
                            else:
                                st.write("No codes found.")
